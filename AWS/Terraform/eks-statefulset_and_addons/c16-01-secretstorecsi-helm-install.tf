...
resource "helm_release" "secrets_store_csi_driver" {
  ...
  set = [
    ...
    # Request a projected service account token for the CSI driver so kubelet
    # passes pod identity tokens to secrets-store.csi.k8s.io during volume mount.
    # This is required for AWS Secrets Store CSI with EKS Pod Identity.
    {

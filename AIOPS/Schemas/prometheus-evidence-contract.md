...
- AMP workspace is created in `Observability/OpenTelemetry_terraform/c7_amp_prometheus_workspace.tf`.
- AWS region is `us-east-1` in `Observability/OpenTelemetry_terraform/terraform.tfvars`.
- ADOT remote-writes Prometheus metrics to AMP in
  `Observability/OpenTelemetry_AMP_AMG/01_adot_collector_prometheus_full_k8s_cluster.yaml`.
...
- Kube State Metrics and Prometheus Node Exporter are installed as EKS add-ons in
  `Observability/OpenTelemetry_terraform/c6_07_eks_addon_kube_state_metrics.tf`
  and `Observability/OpenTelemetry_terraform/c6_06_eks_addon_prometheus_node_exporter.tf`.
